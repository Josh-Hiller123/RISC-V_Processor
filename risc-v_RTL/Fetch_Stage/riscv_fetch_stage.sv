...
riscv_pc #(.PC_INITIAL(PC_INITIAL)) pc_wiring (
.i_clk(i_clk), 
.i_nrst(i_nrst), 
.i_id_jump_target(i_id_jump_target), 
.i_mem_jump_target(i_mem_jump_target), 
.i_ALU(i_ALU), 
...
`ifdef RVFI 
    ...
    assign valid_instruct = o_icache_miss ? 1'b0 : 1'b1; 
    assign o_if_id_next.valid_instruct = valid_instruct;
`endif
